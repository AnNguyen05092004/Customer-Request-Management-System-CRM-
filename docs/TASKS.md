...
- Trạng thái: `RequestStatsService` + `GET /api/requests/stats` (thêm vào `RequestController`, `@PreAuthorize("hasRole('ADMIN')")`) đã code xong, có unit test cho logic tổng hợp (total=0 → completionRate=0; byCategory/byDeveloper). CI thật (GitHub Actions, PR #5) đã chạy full `mvn -B verify` bao gồm integration test Testcontainers PostgreSQL (`AuthMemberIT`, `FoundationIT`, `RequestWorkflowIT`) — tất cả xanh. Đã smoke test thủ công trên `docker compose up` với seed data thật: `GET /stats` (ADMIN) trả đúng total/byCategory/byDeveloper khớp dữ liệu thật kể cả sau khi assign; CLIENT gọi `/stats` → đúng 403. Giữ `[ ]` chỉ vì chưa có **integration test tự động** (regression-safe) cho riêng case role-403 của `/stats` — hiện chỉ verify thủ công, chưa có test tự động chạy trong CI cho case này (đã thử `@WebMvcTest` nhưng `@PreAuthorize` không enforce trong slice đó).
